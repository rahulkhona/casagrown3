-- ============================================================================
-- Migration: Fix settlement notification text and $0 card charge log
--
-- 1. Change settlement notification from "earnings now available" to
--    "settlement processed, pending funds" — money isn't available until
--    the payout.paid webhook fires and confirm_settlement_funds_received()
--    moves pending_usd → available_usd.
-- 2. Skip $0.00 hold_captured ledger entries for seller-only users who
--    had no card purchases and no hold.
-- ============================================================================

-- We only need to update run_market_settlement's notification + ledger logic.
-- The function is recreated from the previous migration, so we patch it here.
  ...
  LOOP
    ...
    BEGIN
      ...
      IF v_hold IS NOT NULL THEN
        ...
        -- ★ FIX: Only create settlement_captures and ledger entries when there's a non-zero amount
      ...
      -- ★ FIX: Settlement notification — don't say "available", say "processed, pending funds"
      -- The "available for withdrawal" notification is sent later by confirm_settlement_funds_received()
      -- when the payout.paid webhook fires.
