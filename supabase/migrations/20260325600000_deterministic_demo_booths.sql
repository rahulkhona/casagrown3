-- ============================================================================
-- Deterministic Demo Booths + Exclude Flag
-- 1. Uses ST_GeoHash(user_point, 5) to deterministically seed Demo Booths
--    so they never shuffle around for users in the same neighborhood.
-- 2. Adds `exclude_demos` boolean to nearby_booths to allow frontend polling
--    to skip the CPU-heavy generation loop entirely.
-- ============================================================================

SET search_path TO public, extensions;

-- Drop all versions
DROP FUNCTION IF EXISTS nearby_booths(double precision, double precision, double precision, text, text);
DROP FUNCTION IF EXISTS nearby_booths(double precision, double precision, double precision, text, text, numeric, numeric);
DROP FUNCTION IF EXISTS nearby_booths(double precision, double precision, double precision, text, text, numeric, numeric, text);
DROP FUNCTION IF EXISTS nearby_booths(double precision, double precision, double precision, text, text, numeric, numeric, text, text);
DROP FUNCTION IF EXISTS nearby_booths(double precision, double precision, double precision, text, text, numeric, numeric, text, text, boolean);
...
BEGIN
  ...
  -- Check if buyer is in a blocked state (price → $0)
  ...
  -- ── Demo booths: fill remaining slots (ONLY if not excluded) ──
  IF NOT exclude_demos AND v_real_count < v_min_total THEN
    -- Deterministic geohash seed (~5km grid) anchors the booths forever
    v_neighborhood_hash := ST_GeoHash(user_point::geometry, 5);

    FOR v_tmpl IN
      SELECT * FROM demo_booth_templates
      ORDER BY md5(id::text || v_neighborhood_hash)
      LIMIT (v_min_total - v_real_count)
    LOOP
      -- Jitter user coords deterministically
      v_jitter_lat := user_lat + (((abs(hashtext(v_tmpl.id::text || v_neighborhood_hash || 'lat')) % 100) / 10000.0) - 0.005);
      v_jitter_lng := user_lng + (((abs(hashtext(v_tmpl.id::text || v_neighborhood_hash || 'lng')) % 100) / 10000.0) - 0.005);
      v_demo_dist := ROUND(((abs(hashtext(v_tmpl.id::text || v_neighborhood_hash || 'dist')) % 25) / 10.0 + 0.2)::numeric, 1)::DOUBLE PRECISION;

      v_demo_rating := ROUND((v_tmpl.rating_min + ((abs(hashtext(v_tmpl.id::text || v_neighborhood_hash || 'r')) % 100) / 100.0) * (v_tmpl.rating_max - v_tmpl.rating_min))::numeric, 1);
      v_demo_rating_count := v_tmpl.rating_count_min + (abs(hashtext(v_tmpl.id::text || v_neighborhood_hash || 'rc')) % GREATEST(v_tmpl.rating_count_max - v_tmpl.rating_count_min + 1, 1))::integer;
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'unit', dpc.unit,
          'photo', dpc.photo_url,
          'inventory', 5 + (abs(hashtext(dpc.id::text || v_neighborhood_hash)) % 15),
        ...
        ORDER BY md5(dpc.id::text || v_tmpl.id::text || v_neighborhood_hash)
        LIMIT 4 + (abs(hashtext(v_tmpl.id::text || v_neighborhood_hash || 'pc')) % 3)
      ) sub;
      ...
      -- Generate consistent random UUIDs based on the hash so clicking on a demo product doesn't change URLs
      -- Take upper 16 bytes of md5 to form a valid UUID
      -- uuid_in(md5(v_tmpl.id::text || v_neighborhood_hash || 'booth')::cstring)
      v_demo_uid := uuid_in(md5(v_tmpl.id::text || v_neighborhood_hash || 'booth')::cstring);
