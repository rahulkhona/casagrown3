-- ============================================================================
-- Migration: Fix ALL cron jobs with broken URLs
--
-- Root cause: format() bakes URLs at schedule-creation time. If vault secrets
-- don't exist yet, it falls back to localhost. Fix: use dynamic vault
-- resolution at EXECUTION time via inline subqueries (no format()).
--
-- Also: removes duplicate jobs, fixes settlement timing, adds safety-net retry,
-- and hardens run_market_settlement() to auto-expire stale holds.
-- ============================================================================

SET search_path TO public, extensions;

-- ─── 1. Harden run_market_settlement: expire stale holds before processing ───
...
BEGIN
  ...
  -- *** HARDENING: Expire stale holds before processing ***
  UPDATE market_holds
...
-- ─── Helper function: resolve edge function base URL at runtime ──────────────
-- All cron jobs use this to get the correct URL in any environment.
...
-- Helper: resolve service role key at runtime
...
-- Helper: build standard auth headers for edge function calls
CREATE OR REPLACE FUNCTION edge_fn_headers()
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- casabot-community-starter-job duplicates casabot-starter-post
    BEGIN PERFORM cron.unschedule('casabot-community-starter-job'); EXCEPTION WHEN OTHERS THEN END;
    RAISE NOTICE 'Removed duplicate casabot-community-starter-job';

    -- enrich-communities-job duplicates enrich-communities
    BEGIN PERFORM cron.unschedule('enrich-communities-job'); EXCEPTION WHEN OTHERS THEN END;
    RAISE NOTICE 'Removed duplicate enrich-communities-job';
  END IF;
END $outer$;


-- ─── 3. Fix ALL edge-function cron jobs ──────────────────────────────────────
-- Pattern: every job now calls get_edge_fn_base_url() and edge_fn_headers()
-- at EXECUTION time, never baked via format().

DO $outer$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN

    -- 3a. execute-settlement-captures: 07:05 UTC (12:05 AM PDT, 6 min after settlement)
    BEGIN PERFORM cron.unschedule('execute-settlement-captures'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('execute-settlement-captures', '5 7 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/execute-settlement-captures',
        headers := edge_fn_headers(),
        body := (SELECT jsonb_build_object('settlement_id', id)
                 FROM market_settlements WHERE status = 'funds_pending'
                 ORDER BY created_at DESC LIMIT 1)
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed execute-settlement-captures (07:05 UTC)';

    -- 3b. retry-settlement-captures: safety net every 4 hours
    BEGIN PERFORM cron.unschedule('retry-settlement-captures'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('retry-settlement-captures', '0 */4 * * *',
      $cmd$
      SELECT CASE
        WHEN EXISTS (SELECT 1 FROM market_settlements WHERE status = 'funds_pending')
        THEN net.http_post(
          url := get_edge_fn_base_url() || '/execute-settlement-captures',
          headers := edge_fn_headers(),
          body := (SELECT jsonb_build_object('settlement_id', id)
    ...
    RAISE NOTICE 'Added retry-settlement-captures (every 4h safety net)';

    -- 3c. daily-settlement-digest: 15:00 UTC (8:00 AM PDT)
    BEGIN PERFORM cron.unschedule('daily-settlement-digest'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('daily-settlement-digest', '0 15 * * *',
      $cmd$
      SELECT net.http_post(
    ...
    RAISE NOTICE 'Fixed daily-settlement-digest (15:00 UTC)';

    -- 3d. daily-grower-digest: 17:00 UTC (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('daily-grower-digest'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('daily-grower-digest', '0 17 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/market-cron',
        headers := edge_fn_headers(),
        body := '{"action": "grower_digest"}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed daily-grower-digest';

    -- 3e. casabot-starter-post: 14:00 UTC (keep schedule, fix URL)
    ...
    PERFORM cron.schedule('casabot-starter-post', '0 14 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/casabot-starter-post',
        headers := edge_fn_headers(),
        body := '{}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed casabot-starter-post';

    -- 3f. casabot-auto-reply: every 5 min (keep schedule, fix URL)
    ...
    PERFORM cron.schedule('casabot-auto-reply', '*/5 * * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/casabot-auto-reply',
        headers := edge_fn_headers(),
        body := '{}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed casabot-auto-reply';

    -- 3g. enrich-communities: 04:30 UTC (keep schedule, fix URL)
    ...
    PERFORM cron.schedule('enrich-communities', '30 4 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/enrich-communities',
        headers := edge_fn_headers(),
        body := '{}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed enrich-communities';

    -- 3h. send-market-reminders: every 5 min (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('send-market-reminders'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('send-market-reminders', '*/5 * * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/send-market-reminders',
        headers := edge_fn_headers(),
        body := '{}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed send-market-reminders';

    -- 3i. refresh-donation-projects: 00:05 UTC (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('refresh-donation-projects'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('refresh-donation-projects', '5 0 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/fetch-donation-projects',
        headers := edge_fn_headers(),
        body := '{"refresh": true}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed refresh-donation-projects';

    -- 3j. refresh-giftcard-catalog: 00:00 UTC (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('refresh-giftcard-catalog'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('refresh-giftcard-catalog', '0 0 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/fetch-gift-cards',
        headers := edge_fn_headers(),
        body := '{"refresh": true}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed refresh-giftcard-catalog';

    -- 3k. execute-auto-payouts: 00:30 UTC (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('execute-auto-payouts'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('execute-auto-payouts', '30 0 * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/execute-auto-payouts',
        headers := edge_fn_headers(),
        body := '{"action": "auto_payout"}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed execute-auto-payouts';

    -- 3l. process-redemptions: every 15 min (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('process-redemptions'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('process-redemptions', '*/15 * * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/process-redemptions',
        headers := edge_fn_headers(),
        body := '{"source": "cron"}'::jsonb
      )
      $cmd$
    );
    RAISE NOTICE 'Fixed process-redemptions';

    -- 3m. reconcile-redemptions: every 5 min (keep schedule, fix URL)
    BEGIN PERFORM cron.unschedule('reconcile-redemptions'); EXCEPTION WHEN OTHERS THEN END;
    PERFORM cron.schedule('reconcile-redemptions', '*/5 * * * *',
      $cmd$
      SELECT net.http_post(
        url := get_edge_fn_base_url() || '/market-cron',
        headers := edge_fn_headers(),
    ...
    RAISE NOTICE 'Fixed reconcile-redemptions';
