...
--   2. The buyer's zip code is in the booth's delivery_zipcodes array
-- This applies to both the 'delivery' filter and the 'all' (default) filter.

-- Drop the old 12-param version (no buyer_zip) to prevent function overload ambiguity
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE CASE fulfillment_filter
      WHEN 'delivery' THEN bd.offers_delivery AND (
        bd.dist_miles <= bd.delivery_radius_miles
