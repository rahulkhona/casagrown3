-- Add booth_name + seller/buyer zip to transaction log metadata for receipts
...
BEGIN
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'booth_id', o.booth_id,
      'booth_name', COALESCE(mb.name, 'Default Booth'),
      'seller_name', COALESCE(sp.full_name, 'Seller'),
      'seller_zip', sp.zip_code,
      'buyer_name', COALESCE(bp2.full_name, 'Buyer'),
      'buyer_zip', bp2.zip_code,
      'credit_applied', o.balance_applied_usd,
      'settlement_id', o.settlement_id
    )
  FROM market_orders o
  LEFT JOIN profiles sp ON sp.id = o.seller_id
  LEFT JOIN profiles bp2 ON bp2.id = o.buyer_id
  LEFT JOIN market_booths mb ON mb.id = o.booth_id
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'booth_id', o.booth_id,
      'booth_name', COALESCE(mb.name, 'Default Booth'),
      'seller_name', COALESCE(sp2.full_name, 'Seller'),
      'seller_zip', sp2.zip_code,
      'buyer_name', COALESCE(bp.full_name, 'Buyer'),
      'buyer_zip', bp.zip_code,
      'settlement_id', o.settlement_id
    )
  FROM market_orders o
  LEFT JOIN profiles bp ON bp.id = o.buyer_id
  LEFT JOIN profiles sp2 ON sp2.id = o.seller_id
  LEFT JOIN market_booths mb ON mb.id = o.booth_id
