-- ============================================================================
-- Migration: Wizard Field-Level Analytics
--
-- Adds new event types for granular wizard tracking and creates an RPC
-- function for the metrics portal to query wizard field analytics.
-- ============================================================================

-- 1. Expand the CHECK constraint to allow new wizard event types
...
-- 2. Create index for efficient wizard event queries
CREATE INDEX IF NOT EXISTS idx_crm_page_events_wizard
  ON crm_page_events (page_slug, event_type, occurred_at);

-- 3. RPC: metrics_wizard_field_analytics
--    Returns field interaction rates, AI usage, validation errors,
--    step timing, and abandon points for a given wizard and date range.
DROP FUNCTION IF EXISTS metrics_wizard_field_analytics(date, date, text);
CREATE OR REPLACE FUNCTION metrics_wizard_field_analytics(
  p_start DATE,
  p_end DATE,
  p_wizard TEXT DEFAULT '/create-listing',
...
BEGIN
  WITH filtered_events AS (
    ...
    LEFT JOIN crm_page_visits cv ON cv.session_id = e.session_id
    WHERE e.occurred_at::date >= p_start
      AND e.occurred_at::date <= p_end
      AND (p_state IS NULL OR cv.region = p_state)
      AND (p_city IS NULL OR cv.city ILIKE p_city)
      AND (p_zip IS NULL OR cv.zip_code = p_zip)
  ...
  raw_field_names AS (
    SELECT step, field_name FROM (
      VALUES
        -- /create-listing
        ('/create-listing', 1, 'product_name'), ('/create-listing', 1, 'category'), ('/create-listing', 1, 'description'), ('/create-listing', 1, 'harvest_date'), ('/create-listing', 1, 'email'), ('/create-listing', 1, 'otp_code'), ('/create-listing', 1, 'next_button'), ('/create-listing', 1, 'verify_otp_button'),
        ('/create-listing', 2, 'home_address'), ('/create-listing', 2, 'offers_delivery'), ('/create-listing', 2, 'delivery_radius'), ('/create-listing', 2, 'offers_pickup'), ('/create-listing', 2, 'pickup_address'), ('/create-listing', 2, 'next_button'),
        ('/create-listing', 3, 'quantity'), ('/create-listing', 3, 'unit'), ('/create-listing', 3, 'price'), ('/create-listing', 3, 'next_button'),
        
        -- /join
        ('/join', 1, 'full_name'), ('/join', 1, 'email'), ('/join', 1, 'phone'), ('/join', 1, 'street_address'), ('/join', 1, 'city'), ('/join', 1, 'state_code'), ('/join', 1, 'zip_code'), ('/join', 1, 'next_button'),
        ('/join', 2, 'otp_code'), ('/join', 2, 'verify_otp_button'),
