-- ============================================================================
-- Fix quarantine matching: only show quarantines that affect user's zip/county
-- County-specific quarantines should NOT match users in other counties.
-- Also fix check_quarantine_for_seller to handle ZIP+4 profiles.
-- ============================================================================

-- 1. Fix get_quarantines_for_user: precise county matching
...
BEGIN
  ...
  WHERE qz.is_active = true
    ...
    AND (
      -- County-level: only if user is in that exact county
      (qz.county_id IS NOT NULL AND v_county_id IS NOT NULL AND qz.county_id = v_county_id)
      -- State-wide (no county_id = applies to entire state)
...
-- 2. Fix check_quarantine_for_seller: handle ZIP+4 profiles
...
DECLARE
  ...
  v_zip5 TEXT;
BEGIN
  -- Get 5-digit zip (handles both zip_code and legacy zip_plus4)
  SELECT COALESCE(p.zip_code, LEFT(p.zip_plus4, 5)), COALESCE(p.country_code, 'USA')
  INTO v_zip5, v_country_iso_3
  FROM profiles p
  ...
  -- Resolve jurisdiction from zip_codes
  IF v_zip5 IS NOT NULL THEN
    SELECT z.county_id, COALESCE(ci.state_id, co2.state_id)
    INTO v_county_id, v_state_id
    FROM zip_codes z
  ...
  WHERE qz.is_active = true
    ...
    AND (
      -- County-level: seller's exact county
      (qz.county_id IS NOT NULL AND qz.county_id = v_county_id)
      -- State-wide (no county = entire state)
      OR (qz.state_id IS NOT NULL AND qz.county_id IS NULL AND qz.state_id = v_state_id)
      -- Country-level
      OR (qz.country_iso_3 IS NOT NULL AND qz.state_id IS NULL AND qz.county_id IS NULL
          AND qz.country_iso_3 = v_country_iso_3)
