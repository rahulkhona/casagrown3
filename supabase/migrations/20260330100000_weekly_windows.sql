-- ============================================================================
-- Migration: Weekly fulfillment windows & window-based product expiry
-- ============================================================================
-- Changes:
-- 1. Add weekly_delivery_windows / weekly_pickup_windows JSONB to market_booths
--    - Day-keyed: {"monday": [{id, start, end}], "saturday": [...]}
-- 2. Add delivery_windows / pickup_windows / window_dates to market_products
-- 3. Update nearby_booths to filter by product window dates instead of expires_at
-- 4. Set is_open = true for all booths (deprecated) 
-- ============================================================================

SET search_path TO public, extensions;

-- ──────────────────────────────────────────────────────────────────
-- 1. Booth weekly windows (new columns alongside existing flat ones)
-- ──────────────────────────────────────────────────────────────────
ALTER TABLE market_booths 
  ADD COLUMN IF NOT EXISTS weekly_delivery_windows JSONB DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS weekly_pickup_windows JSONB DEFAULT '{}'::jsonb;

COMMENT ON COLUMN market_booths.weekly_delivery_windows IS 'Day-of-week keyed delivery windows: {"monday":[{id,start,end}],...}';
COMMENT ON COLUMN market_booths.weekly_pickup_windows IS 'Day-of-week keyed pickup windows: {"monday":[{id,start,end}],...}';

-- Deprecate is_open: set all booths to open
UPDATE market_booths SET is_open = true WHERE is_open = false;

-- ──────────────────────────────────────────────────────────────────
-- 2. Product-level windows
-- ──────────────────────────────────────────────────────────────────
ALTER TABLE market_products
  ADD COLUMN IF NOT EXISTS product_delivery_windows JSONB DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS product_pickup_windows JSONB DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS window_dates JSONB DEFAULT '[]'::jsonb;

COMMENT ON COLUMN market_products.product_delivery_windows IS 'Per-product delivery window overrides: [{id,start,end}]';
COMMENT ON COLUMN market_products.product_pickup_windows IS 'Per-product pickup window overrides: [{id,start,end}]';
COMMENT ON COLUMN market_products.window_dates IS 'Array of ISO date strings this product is available: ["2026-03-30","2026-03-31"]';

-- Backfill existing products: set window_dates from market_date
UPDATE market_products
SET window_dates = jsonb_build_array(market_date)
WHERE market_date IS NOT NULL 
  AND (window_dates IS NULL OR window_dates = '[]'::jsonb);

-- ──────────────────────────────────────────────────────────────────
-- 3. Update refresh_product_data to return window info
-- ──────────────────────────────────────────────────────────────────
...
RETURNS TABLE(id UUID, price_usd NUMERIC, inventory INTEGER, is_active BOOLEAN,
              product_delivery_windows JSONB, product_pickup_windows JSONB, window_dates JSONB)
...
-- ──────────────────────────────────────────────────────────────────
-- 4. Update nearby_booths: remove is_open filter, add window info to products
-- ──────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  WITH booth_distances AS (
    ...
    WHERE b.pickup_location IS NOT NULL
      -- Removed: AND b.is_open = true (booths are always open now)
  ...
  SELECT f.id, f.owner_id, f.name, f.description, f.decorative_theme, f.header_image_url,
    ...
    f.pickup_address, -- already display_address from CTE
