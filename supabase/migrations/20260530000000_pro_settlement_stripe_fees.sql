-- ═══════════════════════════════════════════════════════════════
-- Migration: Pro Settlement — Stripe Fee Pass-Through
-- ═══════════════════════════════════════════════════════════════
--
-- Adds columns to track actual Stripe processing fees per order
-- and aggregate them in settlement for Pro sellers who have
-- stripe fee pass-through enabled.
-- ═══════════════════════════════════════════════════════════════

-- ── 1. Order-level Stripe fee tracking ──────────────────────────
ALTER TABLE public.market_orders
  ADD COLUMN IF NOT EXISTS stripe_processing_fee_usd NUMERIC(10,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS stripe_fee_passed_through BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN public.market_orders.stripe_processing_fee_usd
  IS 'Actual Stripe processing fee in USD, fetched from Stripe balance_transaction API after charge succeeds.';
COMMENT ON COLUMN public.market_orders.stripe_fee_passed_through
  IS 'If true, the Stripe processing fee is deducted from the seller payout (Pro sellers with pass-through enabled).';

-- ── 2. Settlement-level Stripe fee aggregation ──────────────────
ALTER TABLE public.user_settlements
  ADD COLUMN IF NOT EXISTS stripe_fees_usd NUMERIC(10,2) NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.user_settlements.stripe_fees_usd
  IS 'Total Stripe processing fees passed through to this seller in this settlement period.';

-- ── 3. Widen market_ledger event_type CHECK to include new settlement events ──
...
ALTER TABLE public.market_ledger
  ...
  CHECK (event_type IN (
    ...
    'hold_placed', 'hold_captured', 'hold_released', 'order_completed',
    'fee_charged', 'refund_issued', 'settlement_credit', 'funds_cleared',
    'payout_sent', 'balance_held', 'balance_released', 'balance_consumed',
    'stripe_transfer_reversed', 'balance_restored',
    -- NEW: settlement event types
    'sale', 'platform_fee', 'purchase', 'refund_received', 'stripe_payout',
    'stripe_fee_passthrough', 'pro_subscription', 'cc_charge',
    -- Additional
    'admin_adjustment'
  ));

-- ── 4. Update run_market_settlement() to account for Stripe fees ──
...
DECLARE
  ...
  v_total_stripe_fees NUMERIC(10,2) := 0;  -- NEW: aggregate stripe fees passed through
...
BEGIN
  ...
  FOR v_user IN
    SELECT u.user_id,
      ...
      COALESCE(SUM(u.stripe_fees), 0) AS stripe_fees  -- NEW
    FROM (
  ...
  LOOP
    ...
    BEGIN
      ...
      -- NEW: Ledger entry for Stripe fee pass-through (Pro sellers only)
      ...
      END
      || CASE WHEN v_user.stripe_fees > 0 THEN ' Stripe fees: -$' || ROUND(v_user.stripe_fees, 2) || '.' ELSE '' END  -- NEW
      ...
      v_total_stripe_fees := v_total_stripe_fees + v_user.stripe_fees;  -- NEW
  ...
  v_reconciliation := jsonb_build_object(
    ...
    'total_stripe_fees_passthrough_usd', v_total_stripe_fees,  -- NEW
    'stale_holds_expired', v_expired_count);
