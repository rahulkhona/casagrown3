-- ============================================================================
-- Migration: Booth Open/Close Toggle
-- Adds is_open column to market_booths so sellers can control whether their
-- booth appears in browse results on market day.
-- Also updates nearby_booths RPC to filter out closed booths.
-- ============================================================================

-- 1. Add is_open column (defaults to true — existing booths stay open)
ALTER TABLE market_booths ADD COLUMN IF NOT EXISTS is_open BOOLEAN DEFAULT true;

COMMENT ON COLUMN market_booths.is_open IS 'Seller manually opens/closes booth for next market day. Closed booths are hidden from browse.';

-- 2. Update nearby_booths to filter out closed booths
