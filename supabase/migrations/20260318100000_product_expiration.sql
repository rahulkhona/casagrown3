-- ============================================================================
-- Product Expiration: products expire after their market_date passes
--
-- 1. Update nearby_booths to filter out expired products (market_date < today)
-- 2. Update create_order to reject orders for expired products
-- 3. Update refresh_product_data to mark expired products
-- ============================================================================

-- ============================================================
-- 1. Update nearby_booths: filter expired products
-- ============================================================
...
BEGIN
  ...
  products AS (
    SELECT mp.seller_id,
      -- Only count non-expired active products
      COUNT(*) FILTER (WHERE mp.is_active AND mp.market_date >= CURRENT_DATE)::BIGINT AS total_count,
      COALESCE(
        jsonb_agg(
          jsonb_build_object(
          ...
          AND mp.market_date >= CURRENT_DATE  -- Exclude expired products
...
-- ============================================================
-- 2. Update refresh_product_data to include market_date
--    so the UI polling can detect expired products
-- ============================================================
...
LANGUAGE sql SECURITY DEFINER AS $$
  SELECT mp.id, mp.price_usd, mp.inventory,
    -- Mark as inactive if expired or flagged
    (mp.is_active AND mp.market_date >= CURRENT_DATE) AS is_active
