-- ============================================================================
-- Migration: Enforce AI moderation approval for public marketplace visibility
-- ============================================================================

SET search_path TO public, extensions;

-- 1. Backfill existing live products: if they are currently active and not flagged, approve them
UPDATE market_products
SET moderation_status = 'approved'
WHERE is_active = true AND (moderation_status IS NULL OR moderation_status = 'pending');

-- 2. Update refresh_product_data to rigorously enforce moderation_status
...
BEGIN
  ...
  SELECT mp.id, mp.price_usd, mp.inventory,
    (mp.is_active
      AND NOT mp.is_draft
      AND mp.moderation_status = 'approved'
...
-- 3. Update nearby_booths to rigorously require moderation_status = 'approved' for public matched products
...
BEGIN
  ...
  products AS (
    SELECT mp.seller_id,
      COUNT(*) FILTER (WHERE mp.is_active AND NOT mp.is_draft AND mp.moderation_status = 'approved'
        AND (v_never_expire OR mp.expires_at IS NULL OR mp.expires_at > now())
        AND NOT EXISTS (
      ...
      COALESCE(
        ...
        ) FILTER (WHERE mp.is_active AND NOT mp.is_draft AND mp.moderation_status = 'approved'
          AND (v_never_expire OR mp.expires_at IS NULL OR mp.expires_at > now())
