-- Migration: Upgrade run_market_settlement to support Stripe Connect Netting
-- Created: 2026-05-20
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Log offsetting payout_sent debit ledger entry of size -v_net to net their virtual wallet impact to exactly $0.00
      ...
      VALUES (
        ...
        v_hold_released, 
        v_net, 
        CASE WHEN v_stripe_active AND v_net > 0 THEN 'stripe_transfer_pending' ELSE 'pending' END
      );

      -- Calculate virtual net credit to adjust pending_usd balance
      v_virtual_net_credit := CASE WHEN v_stripe_active AND v_net > 0 THEN 0 ELSE GREATEST(v_net, 0) END;
      ...
      v_notif_content := CASE
        WHEN v_stripe_active AND v_net > 0 THEN '📋 Settlement processed: $' || ROUND(v_net, 2) || ' in earnings will be deposited directly to your bank account via Stripe.'
