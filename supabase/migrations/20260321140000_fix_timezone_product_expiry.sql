-- ============================================================================
-- Fix timezone bug: CURRENT_DATE uses UTC, but product market_dates are Pacific
-- After 5pm PDT (midnight UTC), all products appeared expired
-- Fix: use (now() AT TIME ZONE 'America/Los_Angeles')::date for all date comparisons
-- ============================================================================
...
BEGIN
  ...
  -- Use Pacific time for date comparisons (products are listed per market day in Pacific)
  ...
  WITH booth_distances AS (
    ...
    WHERE b.pickup_location IS NOT NULL
      AND b.is_open = true  -- Only show open booths
  ...
  products AS (
    SELECT mp.seller_id,
      COUNT(*) FILTER (WHERE mp.is_active AND (v_never_expire OR mp.market_date >= v_today))::BIGINT AS total_count,
      COALESCE(
        jsonb_agg(
          jsonb_build_object(
          ...
          AND (v_never_expire OR mp.market_date >= v_today)
...
-- Also fix refresh_product_data (used by cart page to check product availability)
...
-- Add top_up_count column to market_holds (used by market-hold edge function)
ALTER TABLE market_holds ADD COLUMN IF NOT EXISTS top_up_count integer DEFAULT 0;
