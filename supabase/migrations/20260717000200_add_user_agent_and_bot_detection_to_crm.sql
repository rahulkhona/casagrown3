-- Add user_agent and is_bot columns to crm_page_visits
ALTER TABLE public.crm_page_visits ADD COLUMN IF NOT EXISTS user_agent TEXT;
ALTER TABLE public.crm_page_visits ADD COLUMN IF NOT EXISTS is_bot BOOLEAN NOT NULL DEFAULT false;

-- Create an index on is_bot to optimize filtering
CREATE INDEX IF NOT EXISTS idx_crm_page_visits_is_bot ON public.crm_page_visits(is_bot);

-- Redefine metrics_crm_lead_funnel to filter out bots now that is_bot exists
...
RETURNS JSONB LANGUAGE sql SECURITY DEFINER AS $$
  SELECT jsonb_build_object(
    'page_visits',    (SELECT COUNT(*) FROM crm_page_visits WHERE is_bot = false AND visited_at BETWEEN p_start::TIMESTAMPTZ AND p_end::TIMESTAMPTZ),
    'form_starts',    (SELECT COUNT(*) FROM crm_page_events e JOIN crm_page_visits v ON v.session_id = e.session_id WHERE v.is_bot = false AND e.event_type = 'form_start' AND e.occurred_at BETWEEN p_start::TIMESTAMPTZ AND p_end::TIMESTAMPTZ),
    'form_abandons',  (SELECT COUNT(*) FROM crm_page_events e JOIN crm_page_visits v ON v.session_id = e.session_id WHERE v.is_bot = false AND e.event_type = 'form_abandon' AND e.occurred_at BETWEEN p_start::TIMESTAMPTZ AND p_end::TIMESTAMPTZ),
...
-- Redefine metrics_crm_landing_pages to filter out bots
  ...
  SELECT jsonb_build_object(
    'pages', COALESCE((
      SELECT jsonb_agg(row_to_json(t)) FROM (
        ...
        WHERE is_bot = false
          AND visited_at BETWEEN p_start::TIMESTAMPTZ AND p_end::TIMESTAMPTZ
...
-- Redefine metrics_crm_traffic_sources to filter out bots
...
-- Redefine metrics_crm_ab_results to filter out bots
...
-- Redefine metrics_wizard_field_analytics to filter out bots
...
-- Redefine metrics_wizard_dropoffs to filter out bots
...
BEGIN
  ...
  SELECT 
    ...
    f.step_nm,
    f.visits,
    ROUND((f.visits::NUMERIC / NULLIF(tc.max_v, 0) * 100), 1) AS pct_of_top
  FROM funnel f
