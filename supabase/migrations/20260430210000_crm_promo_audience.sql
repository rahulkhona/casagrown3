-- Add Audience Targeting to Promotions

BEGIN;

-- 1. Add audience_id to crm_promotions
ALTER TABLE public.crm_promotions 
ADD COLUMN audience_id UUID REFERENCES public.crm_audiences(id) ON DELETE SET NULL;

-- 2. Create the crm_check_promo_eligibility function
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM profiles WHERE email = p_email) INTO v_is_registered;
  ...
  IF v_promo.audience_id IS NOT NULL THEN
    ...
    IF v_audience_rpc IS NOT NULL THEN
      EXECUTE format('SELECT EXISTS (SELECT 1 FROM %I() WHERE email = $1)', v_audience_rpc)
...
-- 3. Update crm_enroll_in_promotion to strictly enforce audience
