-- ============================================================================
-- Fix: Remove quarantine filtering from nearby_booths search RPC
-- ============================================================================
-- Quarantine zones should only display WARNINGS on the product detail page,
-- NOT block products from appearing in search results. The previous
-- implementation had two NOT EXISTS quarantine clauses that actively filtered
-- products out of the marketplace and community Find panel.
-- ============================================================================

SET search_path TO public, extensions;

-- Drop the existing 12-param function
...
-- Re-create without quarantine filtering
...
BEGIN
  ...
  products AS (
    SELECT mp.seller_id,
      -- Quarantine zones are advisory only (shown as warnings on product detail page).
      -- They do NOT block products from appearing in search results.
