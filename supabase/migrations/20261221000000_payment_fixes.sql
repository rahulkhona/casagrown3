-- 20261221000000_payment_fixes.sql
-- Fixes C-9, C-13, H-7, H-10

-- C-9: Add processing_started_at to redemptions to allow pessimistic locking
ALTER TABLE redemptions ADD COLUMN IF NOT EXISTS processing_started_at TIMESTAMPTZ;
COMMENT ON COLUMN redemptions.processing_started_at IS 'Timestamp when the redemption claim process began via process-selected-payouts, used to avoid double processing and timeout stuck claims.';

-- We need to add 'processing' to redemptions.status if it's an enum or check constraint.
DO $$ 
BEGIN
    ALTER TYPE redemption_status ADD VALUE IF NOT EXISTS 'processing';
EXCEPTION
    WHEN duplicate_object THEN null;
    WHEN undefined_object THEN null;
END $$;

-- H-10: Make invoice.paid idempotent
ALTER TABLE subscription_receipts ADD COLUMN IF NOT EXISTS stripe_invoice_id TEXT;
COMMENT ON COLUMN subscription_receipts.stripe_invoice_id IS 'The ID of the Stripe Invoice associated with this receipt, used for idempotency.';
CREATE UNIQUE INDEX IF NOT EXISTS uq_sub_receipts_invoice
  ON subscription_receipts (stripe_invoice_id) WHERE stripe_invoice_id IS NOT NULL;


-- C-13: Refund balance on auto-cancel in PATH 2, 3, 4 of auto_complete_delivered_orders()
...
BEGIN
  ...
  -- ────────────────────────────────────────────
  FOR v_rec IN
    SELECT mo.id, mo.buyer_id, mo.seller_id, mo.product_name, mo.product_id, mo.quantity, mo.balance_applied_usd, mo.hold_id
  ...
  LOOP
    ...
    IF v_window_end IS NOT NULL AND v_window_end + interval '24 hours' <= now() THEN
      ...
      -- Refund balance applied (C-13 fix)
      IF v_rec.balance_applied_usd IS NOT NULL AND v_rec.balance_applied_usd > 0 THEN
        PERFORM refund_buyer_balance(
          v_rec.buyer_id,
          ROUND(v_rec.balance_applied_usd * 100)::INTEGER
        );
        IF v_rec.hold_id IS NOT NULL THEN
          UPDATE market_holds
            SET spent_amount_cents = GREATEST(0, spent_amount_cents - ROUND(v_rec.balance_applied_usd * 100)::INTEGER)
            WHERE id = v_rec.hold_id;
        END IF;
      END IF;
...
-- H-7: run_market_settlement should settle ONLY completed orders
