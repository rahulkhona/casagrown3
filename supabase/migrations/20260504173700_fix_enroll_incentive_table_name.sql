-- ============================================================
-- FIX: crm_enroll_in_promotion references non-existent table
--
-- Bug: migrations 20260425009000 and 20260430220000 overwrote
-- crm_enroll_in_promotion to INSERT INTO user_recurring_incentives
-- which does not exist. The correct table is user_incentives
-- (created in 20260421110000). Also fixes:
--   - column name: frequency → expiration_frequency
--   - removes invalid ON CONFLICT (user_id) — users can have
--     multiple incentives from different promotions
-- ============================================================
...
BEGIN
  ...
  -- FIX: uses correct table 'user_incentives' (not 'user_recurring_incentives')
  --      and correct column 'expiration_frequency' (not 'frequency')
  ...
  IF FOUND THEN
    ...
    -- Insert into user_incentives (correct table name)
