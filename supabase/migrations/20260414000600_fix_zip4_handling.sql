...
DECLARE
  ...
  v_zip5 TEXT;
BEGIN
  -- Extract 5-digit zip from profile (handles ZIP+4 like '95120-1848')
  SELECT LEFT(p.zip_code, 5), COALESCE(p.country_code, 'USA')
  INTO v_zip5, v_country_iso_3
  FROM profiles p
  WHERE p.id = p_user_id AND p.zip_code IS NOT NULL;
