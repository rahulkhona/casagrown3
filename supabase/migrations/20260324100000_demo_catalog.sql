-- ============================================================================
-- Demo Booth Catalog — prevents empty shelf syndrome
-- Adds demo booth templates + product catalog, modifies nearby_booths to
-- inject demo booths when real booth count < configurable threshold.
-- ============================================================================

SET search_path TO public, extensions;

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Schema: configurable threshold (no is_demo on real tables — demo data
--    lives entirely in separate demo_booth_templates + demo_product_catalog)
-- ─────────────────────────────────────────────────────────────────────────────

-- Configurable threshold: minimum booths to show (real + demo fill)
ALTER TABLE market_settings
  ADD COLUMN IF NOT EXISTS demo_booth_min_total INTEGER NOT NULL DEFAULT 12;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Demo Booth Templates — diverse, realistic seller names
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS demo_booth_templates (
  id SERIAL PRIMARY KEY,
  seller_name TEXT NOT NULL,
  booth_name TEXT NOT NULL,
  description TEXT NOT NULL,
  decorative_theme TEXT NOT NULL DEFAULT 'garden',
  delivery_radius_miles INTEGER NOT NULL DEFAULT 5,
  -- Fake but plausible rating range
  rating_min NUMERIC(2,1) NOT NULL DEFAULT 4.0,
  rating_max NUMERIC(2,1) NOT NULL DEFAULT 5.0,
  rating_count_min INTEGER NOT NULL DEFAULT 8,
  rating_count_max INTEGER NOT NULL DEFAULT 25
);

-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Demo Product Catalog — ~1000 products across valid categories
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS demo_product_catalog (
  id SERIAL PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_demo_product_catalog_category ON demo_product_catalog(category);

-- ─────────────────────────────────────────────────────────────────────────────
-- 4. Modified nearby_booths — inject demo booths below threshold
-- Must DROP and recreate because RETURNS TABLE columns changed (added is_demo)
-- ─────────────────────────────────────────────────────────────────────────────
-- Drop ALL overloaded versions to avoid "function is not unique" errors
...
BEGIN
  ...
  -- Count how many real results we returned
  GET DIAGNOSTICS v_real_count = ROW_COUNT;

  -- ── Demo booths: fill remaining slots ──
  IF v_real_count < v_min_total THEN
    FOR v_tmpl IN
      SELECT * FROM demo_booth_templates ORDER BY random() LIMIT (v_min_total - v_real_count)
    LOOP
      -- Jitter user coords ±0.003–0.005° (~0.2–0.35 miles)
      ...
      -- Random rating within template range
      ...
      -- Pick 4-6 random products, apply filters + state pricing
      ...
      FROM (
        ...
        ORDER BY random()
        LIMIT (4 + floor(random() * 3))::int  -- 4 to 6 products
      ) sub;

      -- Skip this demo booth if filters eliminated all products
      IF jsonb_array_length(v_demo_products) = 0 THEN
        CONTINUE;
      END IF;

      -- Return demo booth row
