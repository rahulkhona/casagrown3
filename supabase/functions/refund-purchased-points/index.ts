...
serveWithCors(async (req, { supabase, env, corsHeaders }) => {
  ...
  const { data: bucket, error: bucketError } = await supabase
    ...
    .select(
      `*, payment_transactions ( stripe_payment_intent_id, service_fee_cents, metadata )`,
    )
    .eq("id", bucketId)
    .eq("user_id", userId)
    ...
    if (!userPaidStripeFee) {
      ...
      if (feeData) {
        const calculatedRestockingFee = Math.round(
          refundAmountCents * (feeData.transaction_fee_percent / 100) +
        ...
        if (calculatedRestockingFee >= refundAmountCents) {
          ...
          finalRefundCents = refundAmountCents - calculatedRestockingFee;
        }
      }
    }
      ...
      await processDatabaseRefund(
        ...
        false,
        {
          refund_method: `Refunded to card ending in ${
            bucket.payment_transactions?.metadata?.card_last4 ||
            bucket.payment_transactions?.metadata?.last4 ||
            "..."
          }`,
          refund_usd_cents: finalRefundCents,
          fee_deducted_cents: refundAmountCents - finalRefundCents,
        },
      );
      return jsonOk(
        {
  ...
  if (fallbackChoice === "venmo") {
    ...
    const { error: redemptionError, data: redemptionData } = await supabaseAdmin
      .from("redemptions")
      .insert({
        user_id: userId,
        item_id: null,
    ...
    if (redemptionError) {
      ...
      return jsonError(
        "Internal error queuing Venmo refund: " + redemptionError.message,
        corsHeaders,
        500,
      );
    }
    ...
    await processDatabaseRefund(
      ...
      true,
      {
        ...
        refund_usd_cents: refundAmountCents,
  ...
  if (fallbackChoice === "egift_card") {
    ...
    const { error: redemptionError, data: redemptionData } = await supabaseAdmin
      ...
      .insert({
        ...
        status: "pending",
        provider: brand_name === "Amazon.com" ? "tremendous" : "reloadly",
        metadata: {
    ...
    if (redemptionError || !redemptionData || !redemptionData[0]) {
      console.error("E-Gift Card redemption insert failed", redemptionError);
      return jsonError(
    ...
    await processDatabaseRefund(
      ...
      true,
      {
        ...
        redemption_id: redemptionData[0].id,
        brand_name: brand_name || "Gift Card",
        face_value_cents: face_value_cents || refundAmountCents,
      },
      redemptionData[0].id,
...
async function processDatabaseRefund(
  ...
  // ACID: Use finalize_point_refund RPC to atomically:
  // 1. Insert point_ledger debit
  // 2. Update purchased_points_buckets (remaining_amount, status)
  const referenceId = customReferenceId || bucket.payment_transaction_id;
  const metadata = {
    bucket_id: bucket.id,
    is_fallback: isFallback,
    card_last4: bucket.payment_transactions?.metadata?.last4,
    card_brand: bucket.payment_transactions?.metadata?.brand,
    status: isFallback ? "pending" : "completed",
    provider: isFallback
      ? (extraMetadata.refund_method?.includes("Venmo")
        ? "venmo"
        : "tremendous")
      : undefined,
    ...extraMetadata,
  ...
  const { error: rpcError } = await supabase.rpc("finalize_point_refund", {
    p_user_id: userId,
  ...
  if (rpcError) {
    console.error(
      "[REFUND] Critical Error in finalize_point_refund:",
      rpcError,
    );
    throw new Error(`Atomic refund failed: ${rpcError.message}`);
  }
}
