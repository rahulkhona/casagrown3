...
- **Codebase Leverage First Protocol**: Before writing or modifying any feature, page, component, or bug fix, the AI agent MUST thoroughly search and analyze the existing codebase first to discover and reuse existing single sources of truth (e.g. `EXHAUSTIVE_INTERESTS_CATALOG`), canonical schemas, shared utilities, hooks, design tokens, and types. Never invent local ad-hoc dictionaries, duplicate models, or synthetic mock arrays.
...
- **Worktree Git Push Protocol**: When working in a git worktree or feature branch and deploying/pushing to `main`, either: (1) **(Recommended)** Push directly from the current working branch using the refspec `git push origin <current-branch>:main` so you don't need to switch branches, or (2) If switching local branches via `git checkout main` to merge/push, ALWAYS remember to switch back to the feature branch (`git checkout <feature-branch>`) immediately after pushing to preserve the worktree context.
