...
test.describe('Link Generator — URL Presets', () => {
  ...
  test('/create-listing preset is in the Destination Page dropdown', async ({ page }) => {
    const destinationSelect = page.locator('label:has-text("Destination Page")').locator('..').locator('select')
    if (await destinationSelect.count() > 0) {
      const createListingOption = destinationSelect.locator('option', { hasText: /create.*listing/i })
      await expect(createListingOption).toHaveCount(1)
    } else {
      const createListingBtn = page.locator('button', { hasText: /create.*listing/i })
