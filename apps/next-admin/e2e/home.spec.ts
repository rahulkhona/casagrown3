...
test.describe('Home Page', () => {
  test('should load without client-side errors', async ({ page }) => {
    ...
    const criticalErrors = errors.filter(e =>
      ...
      && !e.includes('VAPID') && !e.includes('manifest')
      && !e.includes('deprecat') && !e.includes('Deprecat')
      && !e.includes('tamagui') && !e.includes('Tamagui')
      && !e.includes('title element') && !e.includes('Hydration')
      && !e.includes('hydrat') && !e.includes('favicon')
      && !e.includes('sw.js') && !e.includes('workbox')
    )
