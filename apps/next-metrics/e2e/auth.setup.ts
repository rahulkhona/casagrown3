/**
 * Metrics Auth Setup — runs once before all metrics E2E tests.
 *
 * Flow:
...
const AUTH_FILE = 'e2e/.auth/metrics.json'
const COOKIE_KEY = 'casagrown_metrics_auth' // Using a generic name or whatever metrics respects; usually it respects sb-127-auth-token or whatever config uses. We'll use the default.
...
setup('authenticate as metrics admin', async ({ page }) => {
  ...
  // 5. Navigate to login page to set origin (metrics app runs on 3004 usually)
  ...
  await page.evaluate(
    ({ cookieKey, accessToken, refreshToken, user }) => {
      ...
      document.cookie = `sb-127-auth-token=${encodeURIComponent(sessionPayload)}; path=/; max-age=34560000; samesite=lax`
