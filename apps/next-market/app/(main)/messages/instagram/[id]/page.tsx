...
export default function InstagramThreadPage() {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setTimeout(() => {
        const form = document.querySelector('.ig-chat-form')
        if (form) {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('auto-reply-seller-chat', {
        body: {
          ...
          senderId: conversation?.ig_sender_id || 'buyer',
          recipientId: user?.id,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`ig-thread-${conversationId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await supabase.functions.invoke('send-instagram-reply', {
        body: {
  ...
  const displayName = conversation?.ig_sender_id
    ? `IG User ${conversation.ig_sender_id.slice(-4)}`
    : 'Instagram Customer'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - 144px)', maxWidth: '100vw', overflow: 'hidden', background: '#f9fafb' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', background: 'white', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
        ...
        <div style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: '#fdf2f8', overflow: 'hidden', marginRight: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
          📸
        </div>
        <div style={{ flexGrow: 1 }}>
          <h2 style={{ fontSize: '1.1rem', margin: 0, color: '#111827', display: 'flex', alignItems: 'center', gap: 6 }}>
            {displayName}
            <span style={{ fontSize: 10, background: '#E1306C', color: 'white', padding: '2px 6px', borderRadius: 8, fontWeight: 700 }}>Instagram</span>
          </h2>
        </div>
      </header>
      ...
      <main style={{ flexGrow: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column' }}>
        ...
        {messages.map((msg, idx) => {
          ...
          const isImage = msg.content && msg.content.startsWith('http') && (
            ...
            msg.content.includes('cdninstagram.com')
          )
              ...
              <div style={{ display: 'flex', justifyContent, marginBottom: 8 }}>
                ...
                {role === 'buyer' && (
                  <div style={{ width: 28, height: 28, marginRight: 8, alignSelf: 'flex-end' }}>
                    <div style={{ width: '100%', height: '100%', borderRadius: '50%', backgroundColor: '#fdf2f8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: '#E1306C', fontWeight: 'bold' }}>📸</div>
                  </div>
                )}
      ...
      {showSuggestions && (
        <div style={{ padding: '0 16px' }}>
          <BotSuggestionBar
            channel="instagram"
            conversationRef={`instagram_${conversationId}`}
      ...
      <footer style={{ background: 'white', padding: '12px 16px', borderTop: '1px solid #e5e7eb', zIndex: 10 }}>
        ...
        <form className="ig-chat-form" onSubmit={handleSend} style={{ display: 'flex', gap: 6, alignItems: 'center', position: 'relative' }}>
          ...
          <textarea
            ...
            placeholder={uploadingMedia ? "Uploading..." : "Reply via Instagram..."}
          ...
          <button 
            ...
            style={{ background: (inputText.trim() && !uploadingMedia) ? '#E1306C' : '#9ca3af', color: 'white', border: 'none', width: 44, height: 44, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: (inputText.trim() && !uploadingMedia) ? 'pointer' : 'default', transition: 'background 0.2s', flexShrink: 0 }}
          >
