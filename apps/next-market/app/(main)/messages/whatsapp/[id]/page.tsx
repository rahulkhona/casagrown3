...
export default function WhatsAppThreadPage() {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('auto-reply-seller-chat', {
        body: {
          ...
          senderId: conversation?.wa_sender_phone || 'buyer',
          recipientId: user?.id,
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await supabase.functions.invoke('send-whatsapp-reply', {
        body: {
  ...
  const handleBotSend = (text: string) => {
    ...
    setShowSuggestions(false)
    setTimeout(() => {
      const form = document.querySelector('.wa-chat-form')
      if (form) {
  ...
  const displayName = conversation?.wa_sender_phone
    ? `WA Customer ${conversation.wa_sender_phone.slice(-4)}`
    : 'WhatsApp Customer'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - 144px)', maxWidth: '100vw', overflow: 'hidden', background: '#f9fafb' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', background: 'white', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
        ...
        <div style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: '#f0fdf4', overflow: 'hidden', marginRight: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
          🟢
        </div>
        <div style={{ flexGrow: 1 }}>
          <h2 style={{ fontSize: '1.1rem', margin: 0, color: '#111827', display: 'flex', alignItems: 'center', gap: 6 }}>
            {displayName}
            <span style={{ fontSize: 10, background: '#25D366', color: 'white', padding: '2px 6px', borderRadius: 8, fontWeight: 700 }}>WhatsApp</span>
          </h2>
        </div>
      </header>
              ...
              <div style={{ display: 'flex', justifyContent, marginBottom: 8 }}>
                ...
                {role === 'buyer' && (
                  <div style={{ width: 28, height: 28, marginRight: 8, alignSelf: 'flex-end' }}>
                    <div style={{ width: '100%', height: '100%', borderRadius: '50%', backgroundColor: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: '#25D366', fontWeight: 'bold' }}>🟢</div>
                  </div>
                )}
      ...
      {showSuggestions && (
        <div style={{ padding: '0 16px' }}>
          <BotSuggestionBar
            channel="whatsapp"
            conversationRef={`whatsapp_${conversationId}`}
      ...
      <footer style={{ background: 'white', padding: '12px 16px', borderTop: '1px solid #e5e7eb', zIndex: 10 }}>
        ...
        <form className="wa-chat-form" onSubmit={handleSend} style={{ display: 'flex', gap: 6, alignItems: 'center', position: 'relative' }}>
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            <button type="button" title="Emojis" onClick={() => setShowEmojiPicker(!showEmojiPicker)} disabled={sending} style={{ background: '#f3f4f6', color: '#4b5563', border: 'none', width: 44, height: 44, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: sending ? 0.5 : 1 }}>
          ...
          <button 
            ...
            style={{ background: inputText.trim() ? '#25D366' : '#9ca3af', color: 'white', border: 'none', width: 44, height: 44, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: inputText.trim() ? 'pointer' : 'default', transition: 'background 0.2s', flexShrink: 0 }}
          >
