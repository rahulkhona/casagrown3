  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #f0fdf4 0%, #ffffff 40%)', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '20px 16px 40px' }}>
        ...
        {/* Question image */}
        {share!.image_url && (
          <div style={{ marginBottom: 12, borderRadius: 12, overflow: 'hidden', maxWidth: 240 }}>
            <img src={share!.image_url} alt="Plant photo" style={{ width: '100%', height: 'auto', display: 'block', borderRadius: 12 }} />
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 24 }}>
          ...
          <div style={{ flex: 1 }}>
            {share!.bot_response && share!.bot_response.trim().length > 0 && (
              <div style={{ background: 'white', borderRadius: '16px 16px 16px 4px', padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', fontSize: 14, lineHeight: 1.6, color: '#111827', marginBottom: share!.actions?.length > 0 ? 8 : 0 }}>
                {renderMarkdown(share!.bot_response)}
              </div>
            )}
            {share!.actions && share!.actions.length > 0 && (
              <div style={{ background: 'white', borderRadius: '16px', padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
                {share!.actions.map((action: any, i: number) => {
                  const d = action.data || {}
                  switch (action.type) {
                    case 'DiagnosisCard':
                      return (
                        <div key={i}>
                          <div style={{ fontWeight: 700, color: '#166534', fontSize: 15, marginBottom: 8 }}>🔬 Plant Diagnosis</div>
                          <div style={{ marginBottom: 6 }}><strong>Diagnosis:</strong> {d.diagnosis}</div>
                          <div style={{ marginBottom: 6 }}><strong>Urgency:</strong> {d.urgency}</div>
                          {(d.remedy_plan || d.remedyPlan) && (
                            <div style={{ background: '#f9fafb', borderRadius: 8, padding: '8px 12px', marginTop: 8 }}>
                              <div style={{ fontWeight: 600, marginBottom: 4 }}>Remedy Plan:</div>
                              <div style={{ fontSize: 13, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{d.remedy_plan || d.remedyPlan}</div>
                            </div>
                          )}
                        </div>
                      )
                    case 'PlantIdentificationCard':
                      return (
                        <div key={i}>
                          <div style={{ fontWeight: 700, color: '#166534', fontSize: 15, marginBottom: 8 }}>🌿 Plant Identification</div>
                          <div style={{ marginBottom: 6 }}><strong>Name:</strong> {d.common_name || d.commonName || d.name}</div>
                          <div style={{ marginBottom: 6 }}><strong>Scientific:</strong> {d.scientific_name || d.scientificName || 'N/A'}</div>
                          {(d.description || d.care_tips || d.careTips) && (
                            <div style={{ fontSize: 13, lineHeight: 1.6, marginTop: 8 }}>{d.description || d.care_tips || d.careTips}</div>
                          )}
                        </div>
                      )
                    case 'RecipeCard':
                      return (
                        <div key={i}>
                          <div style={{ fontWeight: 700, color: '#166534', fontSize: 15, marginBottom: 8 }}>🍽️ {d.name || d.title || 'Recipe'}</div>
                          {d.description && <div style={{ marginBottom: 6, fontSize: 13 }}>{d.description}</div>}
                          {d.ingredients && (
                            <div style={{ background: '#f9fafb', borderRadius: 8, padding: '8px 12px', marginTop: 8 }}>
                              <div style={{ fontWeight: 600, marginBottom: 4 }}>Ingredients:</div>
                              <div style={{ fontSize: 13, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{Array.isArray(d.ingredients) ? d.ingredients.join('\n') : d.ingredients}</div>
                            </div>
                          )}
                        </div>
                      )
