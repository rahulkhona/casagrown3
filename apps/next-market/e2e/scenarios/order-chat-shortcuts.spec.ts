...
test.describe('Order Chat Shortcuts', () => {
  test('Quick replies and ETA picker work correctly', async ({ browser }) => {
    // 1. Setup Data - Always create a fresh pickup order to prevent interference from other tests
    console.log('[CHAT SHORTCUTS] Creating fresh test order...')
    let orderId = execSql(`
      ...
      RETURNING id
    `)
    const idMatch = orderId.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i)
    if (!idMatch) {
      console.log('[CHAT SHORTCUTS] Could not parse test order ID. Skipping. Raw output:', orderId)
      test.skip()
      return
    }
    const cleanOrderId = idMatch[0]
    ...
    await navigateTo(page, `/orders/${cleanOrderId}`)
    ...
    await expect(chatToggle).toBeVisible({ timeout: 15000 })
    ...
    await navigateTo(bethPage, `/orders/${cleanOrderId}`)
    ...
    await expect(bethChatToggle).toBeVisible({ timeout: 15000 })
