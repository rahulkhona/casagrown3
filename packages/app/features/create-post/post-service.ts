...
async function uploadAndLinkMedia(
  ...
  const { error: linkError } = await supabase.from('post_media').insert(linkRows)
 ...
 */
export async function getActiveDelegators(userId: string): Promise<DelegatorInfo[]> {
  ...
  return (data || []).map((d: Record<string, unknown>) => {
    ...
    return {
      ...
      communityName: ((dp?.communities as Record<string, unknown>)?.name as string) || null,
    }
  })
}
...
export async function getPlatformFeePercent(userId: string): Promise<number> {
  const { data, error } = await supabase.rpc('get_platform_fee_for_user', { p_user_id: userId })
...
export async function getUserCommunitiesWithNeighbors(
  ...
  for (const c of communities) {
    ...
    if (c.location && typeof c.location === 'object' && c.location.coordinates) {
...
export async function getUserCommunity(
  ...
  return {
    ...
    communityName:
      (profile.communities as { name?: string })?.name || profile.home_community_h3_index,
  }
}
 ...
 */
export async function getAvailableCategories(communityH3Index?: string): Promise<string[]> {
  ...
  if (error) {
    console.error('Error fetching category restrictions:', JSON.stringify(error))
    return [...ALL_CATEGORIES] // Fail open
  }

  // Collect blocked categories
  const blocked = new Set((restrictions || []).map((r: { category: string }) => r.category))
...
export async function createSellPost(data: SellPostData) {
  ...
  if (data.additionalCommunityH3Indices && data.additionalCommunityH3Indices.length > 0) {
  ...
  const { error: detailError } = await supabase.from('want_to_sell_details').insert(detailPayload)
  ...
  if (data.dropoffDates.length > 0) {
    ...
    const { error: dateError } = await supabase.from('delivery_dates').insert(dateRows)
...
export async function createBuyPost(data: BuyPostData) {
  ...
  const { error: detailError } = await supabase.from('want_to_buy_details').insert(detailRow)
...
export async function buildCommunityMapData(
  ...
  if (Platform.OS === 'web') {
    ...
    const mapData = buildResolveResponseFromIndex(primary.h3Index, primary.name, primary.city || '')
